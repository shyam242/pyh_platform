              ...
              <Card title="Uploaded resume" icon={<FileText size={16} />} accent="#2563EB" accentLite="#EFF6FF"
                action={
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <span onClick={() => fileRef.current?.click()} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12.5, fontWeight: 700, color: O, cursor: verifyLoading ? "not-allowed" : "pointer", padding: "6px 12px", borderRadius: 8, border: `1px solid ${O_MID}`, backgroundColor: O_LITE, opacity: verifyLoading ? 0.6 : 1 }}>
                      <Upload size={12} /> {verifyLoading ? "Uploading..." : profile.resume_file_path ? "Replace" : "Upload"}
                    </span>
                    <input
                      ref={fileRef}
                      type="file"
                      ...
                      onChange={e => { uploadResumeQuick(e.target.files?.[0]); e.target.value = ""; }}
                    />
                  </div>
                }
              >
