...
import { ArrowLeft, Download, Mail, Phone, MapPin, Briefcase, DollarSign, Clock, BookOpen, Award, ExternalLink, Pencil, X, Save, Trash2 } from "lucide-react";
...
export default function AdminBulkCandidateDetailsPage() {
  ...
  const fetchCandidateDetails = async () => {
    try {
      ...
      const found = (response.data || []).find(c => String(c.id) === String(candidateId));
  ...
  const downloadResume = () => {
    if (!candidate?.resume_link) return showError("No resume available for this candidate");
    window.open(candidate.resume_link, "_blank", "noopener,noreferrer");
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f8fafc" }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ position: "relative", background: "#fff", borderRadius: 16, padding: "28px 32px", width: "100%", maxWidth: 700, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.18)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#0f172a" }}>Edit Bulk Candidate</h2>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
              ...
              <Input label="Contact Number" field="contact" form={form} setForm={setForm} />
      ...
      <div style={{ backgroundColor: "white", padding: "1.5rem 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <h1 style={{ margin: 0, fontSize: "1.5rem", fontWeight: "700", color: "#0f172a" }}>Bulk Candidate Details</h1>
        </div>
          ...
          <div>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "1rem", padding: "2rem", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", marginBottom: "1.5rem" }}>
              <div style={{ display: "flex", gap: "1.5rem", marginBottom: "2rem", paddingBottom: "2rem", borderBottom: "2px solid #f1f5f9" }}>
                <div style={{ width: "100px", height: "100px", borderRadius: "0.5rem", backgroundColor: "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2.5rem", fontWeight: "700", color: "#64748b", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: "0 0 1rem", color: "#64748b", fontSize: "0.95rem" }}>
                    {candidate.email}{candidate.contact ? ` • ${candidate.contact}` : ""}
                  </p>
                  <span style={{ display: "inline-block", padding: "0.4rem 0.8rem", backgroundColor: "#eff6ff", color: "#1d4ed8", borderRadius: "0.25rem", fontSize: "0.85rem", fontWeight: "600" }}>
                    Bulk Upload{candidate.role ? ` • ${candidate.role}` : ""}
                  </span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: "2rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  {[
                    ...
                    { icon: Phone, label: "Contact Number", value: candidate.contact },
              ...
              {/* Professional Info */}
              {(candidate.current_company_name || candidate.experience || candidate.current_ctc || candidate.expected_ctc || candidate.notice_period || candidate.offer_in_hand) && (
                <div style={{ marginBottom: "2rem" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                    {[
                      ...
                      candidate.current_ctc && { icon: DollarSign, label: "Current CTC (LPA)", value: candidate.current_ctc },
                      candidate.expected_ctc && { icon: DollarSign, label: "Expected CTC (LPA)", value: candidate.expected_ctc },
              ...
              {(generalSkills.length > 0 || technicalSkills.length > 0 || softSkills.length > 0) && (
                <div style={{ marginBottom: "2rem" }}>
                  ...
                  {[
                    { label: "General Skills", skills: generalSkills, bg: "#eff6ff", color: "#1d4ed8" },
              ...
              {/* Additional Info */}
              {(candidate.reason_for_change || candidate.linkedin) && (
                <div style={{ marginBottom: "2rem" }}>
            ...
            {candidate.resume_link && (
              <div style={{ backgroundColor: "white", borderRadius: "1rem", padding: "1.5rem", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                <button onClick={downloadResume}
                  style={{ width: "100%", padding: "0.875rem", backgroundColor: O, color: "#fff", border: "none", borderRadius: "0.5rem", fontSize: "0.95rem", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "1rem", padding: "1.5rem", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <button onClick={handleDeleteCandidate}
                  style={{ width: "100%", padding: "0.875rem", backgroundColor: "#dc2626", color: "#fff", border: "none", borderRadius: "0.5rem", fontSize: "0.95rem", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
                  <Trash2 size={16} /> Delete Candidate Profile
                </button>
              </div>
            </div>

            {/* Profile Status */}
            <div style={{ backgroundColor: "#eff6ff", borderRadius: "1rem", padding: "1.5rem", border: "1px solid #bfdbfe" }}>
              <h4 style={{ margin: "0 0 0.75rem", fontSize: "0.95rem", fontWeight: "700", color: "#1e3a8a" }}>Profile Source</h4>
              <p style={{ margin: 0, fontSize: "0.85rem", color: "#1e40af", lineHeight: "1.5" }}>
                This candidate was added via bulk CSV/resume upload{candidate.status ? ` — status: ${candidate.status}` : ""}.
              </p>
            </div>
          </div>
        </div>
