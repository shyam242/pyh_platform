...
import { buildObjectKey, uploadBufferToR2, toR2Key, resolveResumeUrl, isR2Key } from "../utils/r2Storage.js";
...
export const verifyProfile = async (req, res) => {
  try {
    ...
    const objectKey = buildObjectKey("resumes/verify", userId, req.file.originalname);
    ...
    const resumeFile = toR2Key(objectKey);
...
export const downloadReferralCv = async (req, res) => {
  try {
    ...
    if (isR2Key(cv_file)) {
      try {
        const signedUrl = await resolveResumeUrl(cv_file);
        await streamRemoteFile(signedUrl, res, `${name}-CV.pdf`);
      } catch (e) {
        console.error("Error proxying R2 CV download:", e);
        res.status(502).json({ error: "Could not fetch the CV from storage. Please try again or ask the referrer to re-upload." });
      }
      return;
    }
...
export const downloadCandidateResume = async (req, res) => {
  try {
    ...
    if (isR2Key(resume)) {
      try {
        const signedUrl = await resolveResumeUrl(resume);
        await streamRemoteFile(signedUrl, res, `${name}-Resume.pdf`);
      } catch (e) {
        console.error("Error proxying R2 resume download:", e);
        res.status(502).json({ error: "Could not fetch the resume from storage. Please try again." });
      }
      return;
    }

    // Legacy record from before the R2 migration — file may or may not
    // still exist on local disk (won't survive a redeploy).
...
export const trackResumeView = async (req, res) => {
  try {
    const recruiterId = req.user.id;
    const { candidateId, candidateName, viewType } = req.body;
